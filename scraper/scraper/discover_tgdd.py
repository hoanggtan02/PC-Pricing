...
def main() -> int:
    ...
    # SKU nào đã có source ở TGDĐ -> đã được Mode B (daily sync) theo dõi giá. Chỉ ghi giá cho
    # SKU MỚI (chưa có trong tập này); sku cũ chỉ refresh URL.
    ...
    for item in found:
        ...
        # SKU cũ chỉ refresh URL qua source_rows — giá của nó Mode B (daily sync) đã cào rồi.
        if is_new:
            price_rows.append(
