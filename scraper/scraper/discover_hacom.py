...
def main() -> int:
    ...
    # Chỉ giữ các sản phẩm ĐÃ KHỚP SKU, rồi kiểm tồn kho cho riêng chúng (tín hiệu OOS chỉ có ở trang
    # sản phẩm — nút "ĐĂNG KÝ MUA"). Kiểm ở đây để không tải trang cho hàng không dùng đến.
    matched = []
    for item in found:
