...
_OLD_LISTING_PATTERN = r"\btray\b|\btrầy\b|\bdemo\b|\btrưng\s+bày\b|\btrung\s+bay\b|\bcũ\b|\bcu\b|\blike\b|\blikenew\b|\bnhập\s+khẩu\b|\bnhap\s+khau\b|\bxách\s+tay\b|\bxach\s+tay\b|\busa\b|\bxước\b|\bxuoc\b|\bcấn\b|\bcan\b|\btrôi\s+bảo\s+hành\b|\btroi\s+bao\s+hanh\b|\bactive\b|\bđã\s+kích\s+hoạt\b|\bda\s+kich\s+hoat\b|\bkhông\s+(?:vỏ\s+)?hộp\b|\bkhong\s+(?:vo\s+)?hop\b|\bđổi\s+trả\b|\bdoi\s+tra\b"
