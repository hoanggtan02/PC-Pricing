...
_OLD_LISTING_PATTERN = r"\btray\b|\btrầy\b|\bdemo\b|\btrưng\s+bày\b|\btrung\s+bay\b|\bcũ\b|\bcu\b|\blike\b|\blikenew\b|nhập\s+khẩu|nhap\s+khau|xách\s+tay|xach\s+tay|\busa\b|xước|xuoc|cấn|can|\btrôi\s+bảo\s+hành\b|\btroi\s+bao\s+hanh\b|\bactive\b|\bđã\s+kích\s+hoạt\b|\bda\s+kich\s+hoat\b|\bkhông\s+(?:vỏ\s+)?hộp\b|\bkhong\s+(?:vo\s+)?hop\b|đổi\s+trả|doi\s+tra"
